...
class Pframe(CompressesModel):
    ...
    def forward(self, coding_frame, dpb, order=0, q_level=None):
        ...
        assert isinstance(q_level, list), ValueError
        assert coding_frame.size(0) % len(q_level) == 0, ValueError

        if order == 0:
            iqs = []
            result = self.i_model.forward(coding_frame, torch.cat([self.i_model.q_scale[q:q+1] for q in q_level], dim=0))
            dpb = {
            ...
            result = self.p_model.forward_one_frame(coding_frame, dpb,
                                                mv_y_q_scale=torch.cat([self.p_model.mv_y_q_scale[q:q+1] for q in q_level], dim=0),
                                                y_q_scale=torch.cat([self.p_model.y_q_scale[q:q+1] for q in q_level], dim=0))
            dpb = result["dpb"]
    ...
    def training_step(self, batch, batch_idx):
        ...
        q_level = [q%4 for q in range(batch.size(0))]
        training_lmda = torch.tensor([{0: 85, 1: 170, 2: 380, 3: 840}[q%4] for q in range(batch.size(0))], dtype=torch.float)
        training_lmda = training_lmda.view(batch.size(0), 1, 1, 1).to(batch.device)
        ...
        for frame_idx in range(0, training_frames):
            ...
            loss = self.args.lmda_scale * (training_lmda * distortion).mean() + rate.mean()
